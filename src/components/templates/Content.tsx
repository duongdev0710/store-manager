...
const arrImg = [
  { id: 1, address: 'TP. Hà Nội', image: 'https://csfood.vn/wp-content/uploads/2016/07/N%C6%B0%E1%BB%9Bc-gi%E1%BA%A3i-kh%C3%A1t-Coca-Cola-lon-250ml.jpg', name: 'Coca Cola', price: 2, unit: '$' },
  { id: 2, address: 'TP. HCM', image: 'https://csfood.vn/wp-content/uploads/2016/04/Nuoc-Giai-Khat-Co-Gas-Mirinda-Huong-Cam.jpg', name: 'Mirinda', price: 2, unit: '$' },
  { id: 3, address: 'TP. Hưng Yên', image: 'https://csfood.vn/wp-content/uploads/2016/07/N%C6%B0%E1%BB%9Bc-gi%E1%BA%A3i-kh%C3%A1t-c%C3%B3-gas-Pepsi-th%C3%B9ng-24-lon-x-330ml.jpg', name: 'Pepsi', price: 3, unit: '$' },
  { id: 4, address: 'TP. Quảng Ninh', image: 'https://csfood.vn/wp-content/uploads/2018/01/Cafe-vi%E1%BB%87t-%C4%91en-u%E1%BB%91ng-li%E1%BB%81n-nescafe-lon-170ml.png', name: 'Cafe Latte', price: 1, unit: '$' },
  { id: 5, address: 'Nước ngoài', image: 'https://csfood.vn/wp-content/uploads/2016/07/N%C6%B0%E1%BB%9Bc-gi%E1%BA%A3i-kh%C3%A1t-h%C6%B0%C6%A1ng-chanh-Sprite-chai-1.5L.jpg', name: 'Sprite', price: 2, unit: '$' },
]
        ...
        {arrProduct.map((item: any, index: number) => {
          return (
            <div key={index} className="col-span-1 border border-black relative flex justify-end flex-col bg-white items-center">
              <div className="bg-[#ddd] w-full p-2">
                <span>{item.name}: </span>
                <span className="text-red-500">{item.price}{item.unit}</span>
                {/*<span className="text-xs text-[#9e9e9e]">{item.address}</span>*/}
              </div>
              <Image
                src={item.image}
                alt={item.name}
              ...
              <Tooltip content={item.name}>
                <button className="hover:h-[50px] hover:font-bold cursor-pointer absolute w-full bg-sky-500 bg-opacity-25 flex items-center" onClick={() => {
                  ...
                  toast.success("Bạn đã thêm 1 sản phẩm vào giỏ hàng")
                }}>
