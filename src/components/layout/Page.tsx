    ...
    <div className="border border-[#9e9e9e] h-screen w-full">
      <div className="grid grid-cols-3 gap-5">
        {arrProduct.map((item: any, index: number) => {
          return (
            <div key={index} className="col-span-1 border border-black relative">
              <Image
                src={item.image}
                alt={item.name}
                width={100}
                height={100}
              />
              <button className="cursor-pointer absolute w-full bg-sky-500 bg-opacity-25 flex items-center" onClick={() => {
                ...
                localStorage.setItem('product', JSON.stringify(itemOrder))
                getItem(itemOrder)
              }}>
